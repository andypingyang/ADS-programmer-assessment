...
# Use a fixed timezone so datetime parsing/derivations are reproducible
Sys.setenv(TZ = "UTC")

# Source SDTM domains from the example package
...
# Standard cleanup: treat blanks as missing (prevents accidental "" comparisons)
...
adsl <- adsl %>%
  mutate(
    ...
    # ITT flag: here defined as “has a planned treatment assignment
    ITTFL = if_else(
      !is.na(ARM) & ARM != "",
      "Y",
      "N"
...
adsl <- adsl %>%
  ...
  mutate(
    ...
    # LSTAVLDT = last available “subject activity” date across domains + last exposure
    LSTAVLDT = pmax(LSTVSDT, LSTAEDT, LSTDSDT, TRTEDT, na.rm = TRUE),
    LSTAVLDT = as.Date(if_else(is.infinite(LSTAVLDT), NA_real_, as.numeric(LSTAVLDT)))
  ) %>%
  select(-LSTVSDT, -LSTAEDT, -LSTDSDT, -TRTEDT)

stopifnot(exists("adsl"))
...
message("Saved ADSL to question_2_adam/output/adsl.rds and adsl.csv")